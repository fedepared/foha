...
export class ColoresService {
  ...
  updateColor (id: number,color: Colores): Observable<Colores> {
    const url = `${apiUrl}/${id}`;
    return this.http.put<Colores>(url, color,httpOptions).pipe(
      tap(_ => console.log(`updated color id=${id}`)),
      catchError(this.handleError<Colores>('update Color'))
    );
  }
